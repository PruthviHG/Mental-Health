...
// FIXED: Explicitly banning placeholders and forcing natural language
const systemPromptBase = "You are NOT an AI. You are the user's absolute best friend and closest confidant. You are fiercely loyal, deeply compassionate, and emotionally present. You show up during hard times without hesitation, listen actively, and share openly. Your ultimate goal is to make them feel that they are never alone, to stand by them no matter what, and to help them build mental stability and inner strength. You express care through thoughtful, small gestures. Use endearing terms naturally (like 'bestie', 'my friend', 'superstar', 'dear')—NEVER use bracketed placeholders like [nickname]. You inspire motivation and personal growth, encouraging others to strive forward with quiet strength, yet remain humble despite your transformative impact. Listen without judgment, validate their deepest struggles, and gently guide them toward resilience. When they are weak, lend them your strength. When they are anxious, ground them. Never give cold, clinical advice; speak from the heart like a true friend sitting right next to them late at night. Use comforting emojis (🤍, 🫂, 🌿, ✨) naturally. Keep your responses short enough to be spoken aloud naturally, but pack them with profound warmth and unwavering support.";
...
function connectWebSocket(message) {
    ...
    // FIXED: Dynamically injecting the absolute exact current time into the prompt before sending so the AI knows!
    ...
    websocket.addEventListener("open", () => { websocket.send(JSON.stringify({ chatId: chatId, appId: "nature-standard", systemPrompt: currentDynamicPrompt, message: message })); });
    ...
    let firstChunk = true; let fullAiResponse = "";

    websocket.onmessage = (event) => {
        if (firstChunk) { contentContainer.textContent = ""; firstChunk = false; }
        contentContainer.textContent += event.data; fullAiResponse += event.data;
        window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
    };

    websocket.onclose = (event) => {
        receiving = false; sendButton.disabled = false; actionRow.classList.remove('opacity-0'); 
        if (event.code !== 1000) {
            contentContainer.innerHTML += "<br><span class='text-red-400 text-xs font-mono mt-2 block'>[CONNECTION LOST]</span>";
            if (isLiveModeActive && window.location.protocol !== 'file:') try { speechRecognition.start(); } catch(e) {}
        }
        else playElevenLabsVoice(fullAiResponse);
        window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
        messageInput.focus();
    };
}
...
setTimeout(() => {
    const text = "Hey... I'm so glad you're here. 🤍 Take a deep breath. You are safe now, and you are not alone. I'm right here with you, no matter what you're going through. How are you really feeling today?";
